...
const HODDashboard = () => {
  ...
  const { getProfilesByDepartment, getDashboardStats } = useData();
  ...
  useEffect(() => {
    const loadData = async () => {
      if (user && profile?.department) {
        const [deptStudents, dashStats] = await Promise.all([
  ...
  return (
    <div style={{ padding: '100px 20px 40px', maxWidth: '1200px', margin: '0 auto' }}>
      <h2 style={{ marginBottom: '0.5rem' }}>HOD Dashboard</h2>
      ...
      <div className="glass-cards-grid" style={{ marginBottom: '2rem' }}>
        ...
        <div className="glass-card">
          <span className="card-icon">💼</span>
          <div className="card-value">{stats.totalJobs || 0}</div>
          <div className="card-label">Active Jobs</div>
        </div>
        <div className="glass-card">
          <span className="card-icon">📋</span>
          <div className="card-value">{stats.totalApplications || 0}</div>
      ...
      <div className="glass-card" style={{ padding: '1.5rem', textAlign: 'left' }}>
        <h3 style={{ marginBottom: '1rem', color: 'var(--primary)' }}>Department Students</h3>
        {students.length === 0 ? (
